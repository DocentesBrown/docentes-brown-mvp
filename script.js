...
function setupEventListeners() {
    ...
    document.getElementById("form-create-taller")?.addEventListener("submit", (e) => {
        ...
        if (selectedIds.length === 0) {
            showMessage(msg, "Seleccioná al menos un docente.", "red");
            return;
        }

        showMessage(msg, "Creando taller...");
        ...
        .then(json => {
            if (json.result === "success") {
                showMessage(msg, "Taller creado exitosamente.", "green");
                setTimeout(() => {
                ...
                showMessage(msg, json.message || "Error al crear taller.", "red");
            }
        })
        .catch(err => {
            console.error(err);
    ...
    document.getElementById("btn-confirm-add-participants")?.addEventListener("click", () => {
        ...
        .then(json => {
            if (json.result === "success") {
                showMessage(msg, "Lista de participantes actualizada.", "green");
                setTimeout(() => {
    ...
    document.getElementById("btn-save-link")?.addEventListener("click", () => {
        ...
        if (!link) {
            alert("Ingresá un enlace primero.");
            return;
        }
    ...
    document.getElementById("btn-save-biblio")?.addEventListener("click", () => {
        ...
        .then(json => {
            if (json.result === "success") {
                showMessage(msg, "Documentación actualizada.", "green");
                setTimeout(() => {
    ...
    apiPost({ action: "getStudentPublications" })
        .then(json => {
            if (json && json.data) {
                ...
                pubsCache = [];
                if (studentsActiveTab === "pubs") renderStudentsList([], "pubs");
            }
        })
        .catch(err => {
            console.error(err);
    ...
    apiPost({ action: "getStudentTutorials" })
        .then(json => {
            if (json && json.data) {
                ...
                tutsCache = [];
                if (studentsActiveTab === "tuts") renderStudentsList([], "tuts");
            }
        })
        .catch(err => {
            console.error(err);
...
function openAddParticipants(id, titulo) {
    ...
    apiPost({ action: "getAllTalleres" })
        ...
        .catch(err => {
            console.error(err);
            document.getElementById("add-docentes-checklist").innerHTML = "<p style='color:red;'>Error al cargar datos.</p>";
        });
}
...
function openBibliografiaTaller(tallerId, titulo, docsJson) {
    ...
    ensureDocumentsLoaded()
        ...
        .catch(err => {
            console.error(err);
            document.getElementById("select-biblio-edit").innerHTML = "<option>Error cargando documentos</option>";
        });
}
...
function filterBiblioSearch(text) {
    const select = document.getElementById("select-biblio-edit");
    if (!select) return;
...
function applyFilters() {
    ...
    apiPost({ action: "getDocuments" })
        .then(json => {
            if (json.result === "success" && json.data) {
                ...
                const filtered = globalDocs.filter(doc => {
                    const matchesText = (doc.titulo || "").toLowerCase().includes(term) || 
        ...
        .catch(err => {
            console.error(err);
            document.getElementById("docs-list-container").innerHTML = "<p style='color:red;'>Error al cargar documentos.</p>";
        });
}
...
function loadMyTalleres(userId) {
    ...
    apiPost({ action: "getMyTalleres", userId })
        ...
        .catch(err => {
            console.error(err);
            container.innerHTML = "<p style='color:red;'>Error al cargar talleres.</p>";
        });
}
...
function showTallerInfo(titulo, link, docsJson) {
    ...
    renderTallerDocsInto(biblioContainer, ids);
}
...
// --- FUNCIONES PARA DOCUMENTACIÓN DE TALLERES ---
...
function renderTallerDocsInto(containerEl, docsIds) {
    ...
    ensureDocumentsLoaded()
        .then(() => {
            ...
            if (!items.length) {
                containerEl.innerHTML = `<p style="color:#777; font-style:italic;">No hay documentación vinculada.</p>`;
                return;
            }
...
window.openBibliografiaTaller = openBibliografiaTaller;
